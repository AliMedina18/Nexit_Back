...
{
    // Emails ahora es una lista (ClienteEmail, ver docs/34) -- "existe por email" es "algún correo de
    // algún cliente coincide", y GetByEmailAsync busca el cliente dueño de ese correo dentro de su lista.
    ...
    public Task<Cliente?> GetByEmailAsync(string email, CancellationToken cancellationToken = default) =>
        DbSet.Include(x => x.Telefonos).Include(x => x.Emails).FirstOrDefaultAsync(x => x.Emails.Any(m => m.Email.ToLower() == email.ToLower()), cancellationToken);
    ...
    public override Task<Cliente?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default) =>
        DbSet.Include(x => x.Telefonos).Include(x => x.Emails).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
    ...
    public override async Task<IReadOnlyList<Cliente>> GetAllAsync(CancellationToken cancellationToken = default) =>
        await DbSet.AsNoTracking().Include(x => x.Telefonos).Include(x => x.Emails).Include(x => x.Proyectos).OrderBy(x => x.Nombre).ToListAsync(cancellationToken);
}
