...
- **Restricción de dominio de correo laboral:** esto **no** lo resuelve Supabase Auth solo — es una regla propia del negocio. Se modela como catálogo, `dominios_correo_permitidos`, en vez de un valor fijo en el código, porque los datos de origen muestran que Next opera en más de un país (Colombia y México) y podría necesitar más de un dominio sin que eso implique tocar el schema. **Actualizado 2026-08-18:** el dominio real confirmado es `agencianextmkt.com` — la usuaria dio el correo real de una compañera que ya trabaja ahí (`yuliana.navarro@agencianextmkt.com`), evidencia mucho más sólida que la anterior. El dominio que estaba antes (`nextexperiencial.com`) venía solo de un placeholder del campo de login en el prototipo HTML (`"nombre@nextexperiencial.com"`), nunca se confirmó con un correo real — se deja en la tabla por si de verdad es válido además del confirmado, pero convendría que la usuaria lo confirme o lo quite. En los Excel también aparece el dominio `nextcolombia.com` en un correo de contacto, pero no hay evidencia de que sea un dominio de *login* del sistema (podría ser solo un correo comercial de cara al cliente) — sigue pendiente confirmarlo con Luisa/el equipo antes de agregarlo a la tabla.
